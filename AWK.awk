...
#	if(rTime<rStartTime) rStartTime=rTime;
	if(rTime>rEndTime) rEndTime=rTime;
	nDropPackets = nSentPackets-nReceivedPackets;
...
printf("%d %15.5f\n", rEndTime,rStartTime);
	...
	rThroughput = nReceivedBytes / (rTime);
...
#		printf("%d %15.5f\n", i, energy_consumption[i]);
		total_energy_consumption = total_energy_consumption + energy_consumption[i];
	}
